...
public:
	...
	asio::error_code open(const protocol_type& protocol, asio::error_code& ec) {
		return this->get_service().open(this->get_implementation(), protocol,
				ec);
	}
  ...
  && !defined(ASIO_ENABLE_CANCELIO)
	...
	asio::error_code bind(const endpoint_type& endpoint, asio::error_code& ec) {
		return this->get_service().bind(this->get_implementation(), endpoint,
				ec);
	}
	...
	ASIO_INITFN_RESULT_TYPE(ConnectHandler,
			void (asio::error_code))async_connect(const endpoint_type& peer_endpoint,

...
public:
	...
	const_buffer(const mutable_buffer& b) :
			data_(asio::detail::buffer_cast_helper(b)), size_(
					asio::detail::buffer_size_helper(b))
#if defined(ASIO_ENABLE_BUFFER_DEBUGGING)
 ...
 */
inline const_buffers_1 buffer(const void* data, std::size_t size_in_bytes) {
 ...
 */
template<typename PodType, typename Allocator>
inline const_buffers_1 buffer(const std::vector<PodType, Allocator>& data) {
	return const_buffers_1(
...
std::size_t buffer_copy(const mutable_buffer& target,
	...
	for (mutable_buffer target_buffer(target);
			buffer_size(target_buffer) && source_iter != source_end;
			++source_iter) {
...
std::size_t buffer_copy(const MutableBufferSequence& target,
	...
	for (const_buffer source_buffer(source);
			buffer_size(source_buffer) && target_iter != target_end;
			++target_iter) {

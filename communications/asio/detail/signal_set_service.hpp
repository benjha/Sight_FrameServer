...
public:
	...
	public:
		// Default constructor.
		registration() :
				signal_number_(0), queue_(0), undelivered_(0), next_in_table_(
						0), prev_in_table_(0), next_in_set_(0) {
		}
	...
	ASIO_DECL
	void fork_service(asio::io_service::fork_event fork_ev);
	...
	ASIO_DECL
	asio::error_code add(implementation_type& impl, int signal_number,
			asio::error_code& ec);

	// Remove a signal to a signal_set.
	ASIO_DECL
	asio::error_code remove(implementation_type& impl, int signal_number,
			asio::error_code& ec);
...
private:
	...
	ASIO_DECL
	static void add_service(signal_set_service* service);

	// Helper function to remove a service from the global signal state.
	ASIO_DECL
	static void remove_service(signal_set_service* service);

	// Helper function to create the pipe descriptors.
	ASIO_DECL
	static void open_descriptors();
	...
	ASIO_DECL
	void start_wait_op(implementation_type& impl, signal_op* op);

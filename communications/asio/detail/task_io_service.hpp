...
class task_io_service: public asio::detail::service_base<task_io_service> {
public:
	...
	ASIO_DECL
	task_io_service(asio::io_service& io_service, std::size_t concurrency_hint =
			0);
...
private:
	...
	ASIO_DECL
	std::size_t do_run_one(mutex::scoped_lock& lock, thread_info& this_thread,
			const asio::error_code& ec);

	// Poll for at most one operation.
	ASIO_DECL
	std::size_t do_poll_one(mutex::scoped_lock& lock, thread_info& this_thread,
			const asio::error_code& ec);

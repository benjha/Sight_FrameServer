...
public:
	coro_handler(basic_yield_context<Handler> ctx) :
			coro_(ctx.coro_.lock()), ca_(ctx.ca_), handler_(ctx.handler_), ready_(
					0), ec_(ctx.ec_), value_(0) {
	}
...
template<typename Handler, typename ReturnType>
struct handler_type<basic_yield_context<Handler>, ReturnType(asio::error_code)> {
	typedef detail::coro_handler<Handler, void> type;
};
...
struct coro_entry_point {
	void operator()(typename basic_yield_context<Handler>::caller_type& ca) {
		...
		const basic_yield_context<Handler> yield(data->coro_, ca,
				data->handler_);

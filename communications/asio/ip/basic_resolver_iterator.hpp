...
public:
	...
	static basic_resolver_iterator create(
		...
		iter.values_.reset(new values_type);
		iter.values_->push_back(
				basic_resolver_entry<InternetProtocol>(endpoint, host_name,
						service_name));
		return iter;
	}
	...
	static basic_resolver_iterator create(EndpointIterator begin,
			EndpointIterator end, const std::string& host_name,
		...
		if (begin != end) {
			...
			for (EndpointIterator ep_iter = begin; ep_iter != end; ++ep_iter) {
				iter.values_->push_back(
						basic_resolver_entry<InternetProtocol>(*ep_iter,

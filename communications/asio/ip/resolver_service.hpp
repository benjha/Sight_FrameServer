...
private:
	...
	service_impl_type;
#else
	typedef asio::detail::resolver_service<InternetProtocol> service_impl_type;
#endif
...
public:
	...
	explicit resolver_service(asio::io_service& io_service) :
			asio::detail::service_base<resolver_service<InternetProtocol> >(
					io_service), service_impl_(io_service) {
	}
	...
	ASIO_INITFN_RESULT_TYPE(ResolveHandler,
			void (asio::error_code, iterator_type))async_resolve(implementation_type& impl, const query_type& query,

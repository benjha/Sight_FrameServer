...
public:
	...
	ASIO_STATIC_CONSTANT(int,
			message_peek = ASIO_OS_DEF(MSG_PEEK));ASIO_STATIC_CONSTANT(int,
			message_out_of_band = ASIO_OS_DEF(MSG_OOB));ASIO_STATIC_CONSTANT(int,
			message_do_not_route = ASIO_OS_DEF(MSG_DONTROUTE));ASIO_STATIC_CONSTANT(int,
	...
	typedef asio::detail::socket_option::boolean<ASIO_OS_DEF(SOL_SOCKET),
			ASIO_OS_DEF(SO_BROADCAST)> broadcast;
#endif
...
#else
	typedef asio::detail::socket_option::boolean<ASIO_OS_DEF(SOL_SOCKET),
			ASIO_OS_DEF(SO_DONTROUTE)> do_not_route;
#endif
...
#else
	typedef asio::detail::socket_option::boolean<ASIO_OS_DEF(SOL_SOCKET),
			ASIO_OS_DEF(SO_KEEPALIVE)> keep_alive;
#endif
	...
	typedef asio::detail::socket_option::integer<ASIO_OS_DEF(SOL_SOCKET),
			ASIO_OS_DEF(SO_SNDBUF)> send_buffer_size;
#endif
...
#else
	typedef asio::detail::socket_option::integer<ASIO_OS_DEF(SOL_SOCKET),
			ASIO_OS_DEF(SO_SNDLOWAT)> send_low_watermark;
#endif
...
#else
	typedef asio::detail::socket_option::integer<ASIO_OS_DEF(SOL_SOCKET),
			ASIO_OS_DEF(SO_RCVBUF)> receive_buffer_size;
#endif
...
#else
	typedef asio::detail::socket_option::integer<ASIO_OS_DEF(SOL_SOCKET),
			ASIO_OS_DEF(SO_RCVLOWAT)> receive_low_watermark;
#endif
...
#else
	typedef asio::detail::socket_option::boolean<ASIO_OS_DEF(SOL_SOCKET),
			ASIO_OS_DEF(SO_REUSEADDR)> reuse_address;
#endif
	...
	typedef asio::detail::socket_option::boolean<
			...
			asio::detail::enable_connection_aborted_option> enable_connection_aborted;
#endif

...
class openssl_context_service: public asio::detail::service_base<
...
public:
	...
	typedef boost::function<
			std::string(std::size_t, context_base::password_purpose)> password_callback_type;
		...
		if (impl != null()) {
			if (impl->default_passwd_callback_userdata) {
				password_callback_type* callback =
						static_cast<password_callback_type*>(impl->default_passwd_callback_userdata);
				delete callback;
	...
	asio::error_code set_options(impl_type& impl, context_base::options o,
			asio::error_code& ec) {
	...
	asio::error_code add_verify_path(impl_type& impl, const std::string& path,
			asio::error_code& ec) {
	...
	asio::error_code use_certificate_file(impl_type& impl,
		...
		if (::SSL_CTX_use_certificate_file(impl, filename.c_str(), file_type)
				!= 1) {
			ec = asio::error::invalid_argument;
			return ec;
		}
	...
	asio::error_code use_private_key_file(impl_type& impl,
		...
		if (::SSL_CTX_use_PrivateKey_file(impl, filename.c_str(), file_type)
				!= 1) {
			ec = asio::error::invalid_argument;
			return ec;
		}
	...
	asio::error_code use_rsa_private_key_file(impl_type& impl,
		...
		if (::SSL_CTX_use_RSAPrivateKey_file(impl, filename.c_str(), file_type)
				!= 1) {
			ec = asio::error::invalid_argument;
			return ec;
		}
	...
	static int password_callback(char* buf, int size, int purpose, void* data) {
		...
		if (data) {
			...
			std::string passwd = (*callback)(static_cast<std::size_t>(size),
					purpose ?
							context_base::for_writing :
							context_base::for_reading);
			*buf = '\0';
